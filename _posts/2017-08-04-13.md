...
The thing is -- there's **`13`**th MDisk, sealed behind the locker in a city bar (named _Akuda_). **And there's no (in-game) way you can unlock it.**
...
Russian localization of the game was done by the company called "Buka Entertainment" or "Бука" (bˈukə -- eng. _bugbear_). For some reason, they decided that simple subtitle translation is not enough and gone as far as to localize every part of the game (i.e. voice acting, some textures, UI and game manual). 
...
1. “Those who forget the past are doomed to repeat it”. Two of the existing implementations are both websites, just like good ol' Darkroom was, and, thus, one day they can suddenly go down, just like their predcessor. <br> It would be nice to create an _offline_ open-sourced alternative, which you can simply back up in your cloud or, in worst case scenario, rewrite by yourself from sources (and this article). <br> When every user has its own copy, truck factor increases. Surviving is spreading.

Welp, after all of this pretentious speeches the initial idea still holds: decipher Internet code, make a tiny decoder, leave its source code on the open ground, and spread a word about it. All aboard!
...
As we're engineers, lets roll out macro machinery to do this task automaticly.
...
<center><img src="{{site.baseurl}}/assets/2017-08-04-13/table.png" alt=""/></center>
...
```
This is a fresh install, I didn't even launched the game. But, Internet code appears _if save slot associated with it is occupied_, i.e. if corresponding save file exists. This means that, in order to find out bytes which are hashed into Internet code, we need to check out which files are created/modified after the first time we saved:
```
>> before: read %./
...
![]({{site.baseurl}}/assets/2017-08-04-13/saving-screen1.png)
...
`01h` bit marks save slot as existing, and `2000h` seems to be a game-specific index of a checkpoint location (_Pey'j's workshop_ in our case). Unhighlighted bytes also seems to contain some game-related info (I suspect Hovercraft races, Looters' caverns and main quests). This leaves us with 20 significant bytes related to the hashing. What are they?
...
* changing bytes at `0000002Ch` doesn't change the locker code in game. Thus, these are _copies_ of an actual locker code stored in a corresponding save file. 
...
== 22355
```
And somewhere in there lies our precious bytes... What should we do? Well, uncle Curt would say "chill up kiddo, those are all numbers", and he would be right! Our locker code is in fact just a bunch of alphabet indexes. We know what they _mean_, but we also want to know what they _are_ under the hood. There're two initial assumptions we can make:
...
```
But wait! We're assuming that encoding starts with `00h` for `A`. What if it starts with `01h`? No problem, let's patch up our `assumptions` with variants there we subtracting **0** instead of **1**:
```
...
Awesome? Aww yiss. Have we gathered some interesting insights from that? Yesiree!
...
I suspect that _every_ in-game locker code could be changed that way (if you know its offset). Anyway, we've opened Akuda closet 2 times in a row, should we stop and go home? 

No way, we just warmed up.

### Decoder
Ah, the essence of the story. Here's the summary of everything we learned about encoding algorithm up to this point:
...
00000000  01 05 07 15 15 07 E1 07 20 00 01 00 00 00 00 00  ......б. .......
00000010  67 02 00 00 00 01 01 00 <b class="red">DE AD BE EF</b> A0 86 01 00  g.......f%.. †..
00000020  A0 86 01 00 A0 86 01 00 00 00 00 00 04 0B 08 12   †.. †..........
00000030  00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00  ................
</pre>
...
<center><img src="{{site.baseurl}}/assets/2017-08-04-13/dump.png" alt=""/></center>

Hey there beefy! Now it's the matter of setting a breakpoint on read access and waiting for the game to choke on it:

<center><img src="{{site.baseurl}}/assets/2017-08-04-13/breakpoint.png" alt=""/></center>
...
Having done all of the above, let me spread the cards on the table:

![]({{site.baseurl}}/assets/2017-08-04-13/encoding.png)

* it's an iterated block cipher with avalanche effect, thereas flipping 1 bit in game stats may change at least 2 letters in Internet code, but changing 1 bit in total playtime drastically changes the whole thing;
* there's a symmetric key (slightly modified array of first 8 prime numbers);
...
And here's an encoding table for the alphabet:
...
It's a typical `[A-Z a-z 0-9]+/` with the following substitutions:
...
And good news are -- encoding is totaly reversible: 
* rewriting 3 rounds of bit-shuffling is a matter of reversing order of rounds and making them backward (i.e. instead of iterating from 1 to X, iterate from X to 1);
* rewriting 1 round of transposition also comes at no cost, since I already organized hardcoded values as a lookup table and we just need to change our query;
* but you probably noticed that salt was **OR**ed in 12th block. `OR` is irreversible operation, so what we gonna do?

Let's look how 12th block is transposed from game data:
```
X & 00000003h << 4
```
12th block is 2 rightmost bits of X shifted 4 positions to the left. And here what is ORed in it:
```
checksum & 000003C0h >> 6
```
4 leftmost bits of checksum shifted 6 positions to the right. Here's the trick:
```
...
```
4 rightmost bits in 12th block are always empty and part of the checksum nicely fits in this vacant place! This means that we just can do
```
blocks/12 & (000003C0h >> 6) << 6 | blocks/13
```
to get our checksum back. In fact we don't actually need to restore it, but otherwise different (and incorrect) Internet codes may produce identical game data. Speaking of correctness, after restoring 5 dwords of game data we need to validate it and ensure that every value fits into its boundaries:

| Game stat | Lower bound | Upper bound |
|:---------:|:-----------:|:-----------:|
| Playtime  | 00:10:00 | 63:14:07 |
| Pallet game trophies | 0 | 2 |
| Animals | 1 | 56 |
| Pearls  | 1 | 88 |
| Yo! Pearl record | 1'40'646 | 2'000'000 |

On encoding, if playtime is larger than upper bound, it will be trimmed down. As for lower bound -- personally I never was able to get below 00:10:12. 

Also, locker code should be in **letter-digit-letter-digit** format, and _actual_ checksum (that resides in 12th and 13th blocks) should be equal to _expected_ (i.e. re-calculated from restored data) checksum, otherwise our Internet code is incorrect and we bail out.

Well, this seems like a conclusion of our trip, as we've solved the problem at hand multiple times. Good job!
...
Again, no cyrillic reading skills required (in fact you shouldn't look at the letters at all). They completely messed up with slashes, `0Eh` and `34h` are identical! This means that there's no bijection between two alphabets, and for **n** `/` in Internet code there's **2^n** variants of decoding. Bloody hell!
...
Perhaps `Erase` is mapped to some index too, but I haven't find it (though I only tried **-1** and **37**). In any case, this is a simple and elegant alternative solution to the problem with Buka transliteration -- we don't even need to type anything language-specific!
...
Oh, but wait! The 13th MDisk! I resisted temptation to Google up its content all this time, but now, I guess, is the right moment. Let me just open the locker in a millionth time. 
