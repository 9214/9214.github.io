---
layout: post
title:  "Beyond Good & Evil and the 13th MDisk"
date:   2017-08-31
---
...
Russian localization of the game was done by the company called "Buka Entertainment" or "Бука" (eng. _bugbear_, _bugaboo_). For some reason, they decided that simple subtitle translation is not enough and went as far as to localize every part of the game (i.e. voice acting, some textures, UI and game manual). 
...
As we can see, `1F 02 1E 0E 30 3D 3F 22 01 17 09 35 22 23 27 25` is our answer. In this bytes, only 6 rightmost bits are significant (since Internet code alphabet consists of 64 = 2<sup>6</sup> symbols) and 2 leftmost bits are always zero.
...
Again, no cyrillic reading skills required (in fact you shouldn't look at letters at all). They completely messed up with slashes, `0Eh` and `34h` are identical! This means that there's no bijection between two alphabets, and for **n** `/` in Internet code there're <b>2<sup>n</sup></b> variants of decoding. Bloody hell!
